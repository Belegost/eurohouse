...
document.addEventListener("DOMContentLoaded", function (event) {
  var ProjectPage = {
    ...
    setData: function setData() {
      ...
      getJSON("https://api.eurohouse.ca/project/".concat(self.id)).then(function (data) {
        return data;
      }).then(function (data) {
        var imagesID = [];
        ...
        for (var i = 0; i < data.included.length; i++) {
          ...
          if (data.included[i].attributes.orientation) {
            if (self.imgList.lastChild.classList.contains('project__portrait')) {
              var firstImgURI = "https://api.eurohouse.ca/static".concat(data.included[i].attributes.uri);
              ...
              var _firstImgURI = "https://api.eurohouse.ca/static".concat(data.included[i].attributes.uri);
            ...
            var imgURI = "https://api.eurohouse.ca/static".concat(data.included[i].attributes.uri);
        ...
        function getTags() {
          return new Promise(function (resolve, reject) {
            for (var i = 0; i < imagesID.length; i++) {
              getJSON("https://api.eurohouse.ca/image/".concat(imagesID[i])).then(function (image) {
        ...
        getTags().then(function (filterTags) {
          ...
          for (var i = 0; i < filterTags.length; i++) {
            getJSON("https://api.eurohouse.ca/tag/".concat(filterTags[i])).then(function (tag) {
